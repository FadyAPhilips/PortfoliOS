...
- **`CardDefs` is rendered once** (`#sol-defs`) with the four suit symbols and six back patterns; cards reference them by id. The cascade rasterizes each face by inlining those defs into the card's own SVG markup before making a data-URL `Image` — a standalone SVG image can't see `<use>` targets elsewhere in the document.
